  ...
  return (
    status ? (
      <div className="absolute top-18 left-2 w-fit bg-main z-50 flex flex-col items-center gap-4 p-4 border-t rounded-[5px] border-white/10">
        ...
        <Link href={"/trash"} className="flex items-center text-sm text-white font-poppins p-2 border-b-4 border-transparent rounded-[5px] hover:bg-dark-gray-background transition-borders duration-140" onClick={handleMenuClick}><Image className="mr-1 w-[24px] h-[24px]" src={TrashIcon} width={28} height={28} alt="Trash" />Trash</Link>
      </div>
    ) : null
  )
}